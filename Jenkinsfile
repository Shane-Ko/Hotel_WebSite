                ...
                container('git') {
                    withCredentials([usernamePassword(
                        ...
                        sh """
                            ...
                            cd gitops-repo
                            sed -i 's|image: ${HARBOR_URL}/${HARBOR_PROJECT}/${IMAGE_NAME}:.*|image: ${HARBOR_URL}/${HARBOR_PROJECT}/${IMAGE_NAME}:${IMAGE_TAG}|g' apps/hotelweb/deployment.yaml
                            git config user.email "jenkins@kopoctc.kr"
                            git config user.name "Jenkins"
                            git add apps/hotelweb/deployment.yaml
                            git diff --cached --quiet || git commit -m "Update hotelweb image to ${IMAGE_TAG}"
